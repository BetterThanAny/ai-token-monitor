-- Restrict leaderboard snapshot providers to the actively supported set.
--
-- Historical migrations added OpenCode, Kimi, and GLM when those client-side
-- providers existed. The app now only uploads Claude and Codex snapshots.
-- Use NOT VALID so existing historical rows do not block deployment; PostgreSQL
-- still enforces the new CHECK constraint for new or updated rows.
...
check (provider in ('claude', 'codex')) not valid;
...
as $$
begin
  perform sync_device_snapshots(p_provider, p_device_id, p_rows, p_stale_dates);
end;
$$;
